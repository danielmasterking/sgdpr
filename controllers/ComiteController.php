...
class ComiteController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->save();
            Yii::$app->params['uploadPath'] = Yii::$app->basePath . '/web/uploads/comite_fotos/';
            $shortPath                      = '/uploads/comite_fotos/';
            foreach ($image as $img) {
                $archivo=new FotoComite();
                $name    = date('Ymd') . rand(1, 10000) . '' . $img->name;
                $path    = Yii::$app->params['uploadPath'] . $name;
                $archivo->setAttribute('archivo', $shortPath . $name);
                $archivo->setAttribute('id_comite', $model->id);
                $img->saveAs($path);
