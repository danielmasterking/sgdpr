		...
		<table class="table table-striped" style="font-size: 18px;">
			...
			<tbody>
				...
				<tr>
						...
						<?php
							...
							foreach($disp_variable as $row_variable){
				   		    ...
				   		    if($pref_electronica->estado!='abierto'){	               
						   		if(in_array("administrador", $permisos) ){
			                  		echo Html::a('<i class="fa fa-file-pdf-o"></i>',Yii::$app->request->baseUrl.'/prefacturaelectronica/imprimir?id='.$pref_electronica->id,['target'=>'_blank']);
						   		} 
						   	}
				   		?>
					</td>
					...
					<td><?='$ '.number_format(($total_fijo_elect+$total_variable_elect), 0, '.', '.').' COP'?></td>
				</tr>
			<?php endforeach;?>
			</tbody>
		</table>
					...
					<td>
				   		<?php
				   		    ...
				   		    if($admin['estado']!='abierto'){	               
						   		if(in_array("administrador", $permisos) ){
			                  		echo Html::a('<i class="fa fa-file-pdf-o"></i>',Yii::$app->request->baseUrl.'/adminsupervision/imprimir?id='.$admin['id_admin'],['target'=>'_blank']);
						   		} 
						   	}
				   		?>
					</td>
