...
<div class="row">
    ...
    <div class="col-md-4">
        <?php 
        if(/*in_array("coordinador", $permisos) ||*/ in_array("administrador", $permisos) || in_array("coordinador-procesos", $permisos)){?>
            ...
            <tr>
               <!--  <td><?='$ '.number_format($key->presupuesto_seguridad, 0, '.', '.').' COP'?></td>
                <td><?='$ '.number_format($key->presupuesto_riesgo, 0, '.', '.').' COP'?></td> -->
                 <td><?='$ '.number_format($key->presupuesto_activo, 0, '.', '.').' COP'?></td>
                <td><?='$ '.number_format($key->presupuesto_gasto, 0, '.', '.').' COP'?></td>
...
<script>
    ...
    function verifica_asignado(id){
        var respuesta='';
        $.ajax({
            url:"<?php echo Yii::$app->request->baseUrl . '/proyectos/verifica_asignado'; ?>",
            type:'POST',
            dataType:"json",
            cache:false,
            async:false,
